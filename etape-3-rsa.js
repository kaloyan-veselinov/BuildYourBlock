const RSATools = require("./RSATools")

const PUBLIC_KEY_ALICE = `-----BEGIN PUBLIC KEY-----\\nmQIDAQAB\n-----END PUBLIC KEY-----`;
const PRIVATE_KEY_ALICE = `-----BEGIN RSA PRIVATE KEY-----\\nptK3iCr990g4yqQyWNDvXqrdz3BrhsLJrdwYooX+iU2VHxj+dKKP\n-----END RSA PRIVATE KEY-----`;

const calcPublicFromPrivate = RSATools.privateToPublic(PRIVATE_KEY_ALICE);

console.log("Les deux clés doivent être identique :", calcPublicFromPrivate === PUBLIC_KEY_ALICE);

const text = 'Hello RSA!';

const signature = RSATools.sign(text, PRIVATE_KEY_ALICE);
console.log('Signature Alice : ', signature.toString('base64'));

const verify = RSATools.verify(text, signature, PUBLIC_KEY_ALICE);
console.log('Verify Alice : ', verify);
